...
app.post("/signup",(req,res)=>{
    users.createUser(req.body,(err,userobj)=>{
        if(err){
            ...
            console.log("account successfully created: "+userobj.username+"   "+userobj.password);
...
app.post("/login",passport.authenticate("local",{session:false,failureRedirect:"/loginfail"}),(req,res)=>{
    res.end(JSON.stringify({login:true}))
})
...
app.get("/api",passport.authenticate("local",{session:false,failureRedirect:"/login"}),(req,res)=>{
